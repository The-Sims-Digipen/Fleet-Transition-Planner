      ...
      <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-1">
        {fields.map((field) => (
          <label key={field.key} className="min-w-0">
            <span className="mb-1 block break-words text-xs text-zinc-400">
              {field.label}
            </span>
            <input
              className="min-w-0 w-full rounded-md border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm outline-none focus:border-zinc-500"
              type="number"
              step={field.step}
